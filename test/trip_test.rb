require_relative 'test_helper'
require 'time'

describe "Trip class" do
  describe "initialize" do
    before do
      start_time = Time.now - 60 * 60 # 60 minutes
      end_time = start_time + 25 * 60 # 25 minutes
      @trip_data = {
        id: 8,
        driver: RideShare::Driver.new(
          id: 7,
          name: "Minnie Driver",
          vin: "12345678901234567",
          status: :AVAILABLE),
        passenger: RideShare::Passenger.new(
          id: 1,
          name: "Ada",
          phone_number: "412-432-7640"
        ),
        start_time: start_time,
        end_time: end_time,
        cost: 23.45,
        rating: 3
      }
      @trip = RideShare::Trip.new(@trip_data)
    end

    it "is an instance of Trip" do
      expect(@trip).must_be_kind_of RideShare::Trip
    end

    it "stores an instance of passenger" do
      expect(@trip.passenger).must_be_kind_of RideShare::Passenger
    end

    it "stores an instance of driver" do
      skip # Unskip after wave 2
      expect(@trip.driver).must_be_kind_of RideShare::Driver
    end

    it "raises an error for an invalid rating" do
      [-3, 0, 6].each do |rating|
        @trip_data[:rating] = rating
        expect do
          RideShare::Trip.new(@trip_data)
        end.must_raise ArgumentError
      end
    end

    it "raises an error when the end time is before the start time" do

      start_time = Time.now + 60 * 60 # 60 minutes
      end_time = start_time - 25 * 60 # 25 minutes
      @trip_data = {
        id: 8,
        driver: RideShare::Driver.new(
          id: 7,
          name: "Minnie Driver",
          vin: "12345678901234567",
          status: :AVAILABLE),
        passenger: RideShare::Passenger.new(
          id: 1,
          name: "Ada",
          phone_number: "412-432-7640"
        ),
        start_time: start_time,
        end_time: end_time,
        cost: 23.45,
        rating: 3
      }

      expect { RideShare::Trip.new(@trip_data) }.must_raise ArgumentError
    end

    it "raises an error when the end time is equal to the start time" do

      start_time = Time.now + 60 * 60 # 60 minutes
      end_time = start_time
      @trip_data = {
        id: 8,
        driver: RideShare::Driver.new(
          id: 7,
          name: "Minnie Driver",
          vin: "12345678901234567",
          status: :AVAILABLE),
        passenger: RideShare::Passenger.new(
          id: 1,
          name: "Ada",
          phone_number: "412-432-7640"
        ),
        start_time: start_time,
        end_time: end_time,
        cost: 23.45,
        rating: 3
      }

      expect { RideShare::Trip.new(@trip_data) }.must_raise ArgumentError
    end
  end

  describe "duration" do
    before do

      start_time = Time.now # now
      end_time = start_time + 60 # add 1 minute
      @trip_data = {
        id: 8,
        driver: RideShare::Driver.new(
          id: 7,
          name: "Minnie Driver",
          vin: "12345678901234567",
          status: :AVAILABLE),
        passenger: RideShare::Passenger.new(
          id: 1,
          name: "Ada",
          phone_number: "412-432-7640"
        ),
        start_time: start_time,
        end_time: end_time,
        cost: 23.45,
        rating: 3
      }
      @trip = RideShare::Trip.new(@trip_data)
    end

    it "should return a Float" do
      expect(@trip.duration).must_be_kind_of Float
    end

    it "should return duration in seconds" do
      expect(@trip.duration).must_equal 60.0
    end

    it 'remainder should be zero when divided by 60 (60 seconds in a minute)' do
      expect((@trip.duration) % 60).must_equal 0
    end
  end
end
